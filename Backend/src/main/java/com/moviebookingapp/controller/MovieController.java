...
public class MovieController {
	...
	public ResponseEntity<List<Movie>> getMovieByName(@PathVariable String movieName) {
		...
		List<Movie> movieList = movieService.findByMovieNameStartingWith(movieName);
		if (movieList.isEmpty()) {
			log.debug("currently no movies are available");
...
//	}
	...
	private void updateAvailableTicketsInMovie(String moviename, String theatreName, Integer noOfTickets) {
		...
		Movie dup = movie;
		int availableTickets=movie.getNoOfTicketsAvailable() - noOfTickets;
		movie.setNoOfTicketsAvailable(availableTickets);
		if(availableTickets==0) {
			movie.setTicketsStatus("SOLD OUT");
		}
		else {
